...
List getMuti(List L1, List L2) {
	...
	*/
	...
	L->next = NULL;
	L = temp;//这时候L是链首
	List Lb = L2->next;//注意这里Lb不是头指针，而是第一个节点
	bool flag = true;
	/*
	在这里说明一下 flag 在这里的作用：
	用于标记，在下面的程序中，是否进行了，  if( > ) 和 if( == )里面的操作，
	若进行了，则flag就变为 false，则不用进行， while(.....){.....},结束后的第一条语句，
	这条语句的含义是，在L中没有找到，这个 待插入项 的位置，所以将其置于l的尾部。//就是这个是最小项
	*/
	while (Lb && Lb->next) {
		La = L1;
		while (La->next)
		{
			...
			s->exp = La->next->exp + Lb->next->exp;//L1的第一个数据乘以L2第二个；
			while (L->next)
			{
				if (s->exp > L->next->exp)//新节点的 指数大于L的指数
				{
				...
				else if (s->exp == L->next->exp)//相等的话
				{
					if (s->coef + L->next->coef == 0)//抵消
					{
						List val = L->next->next;//L的这个节点删除掉
						L->next = val;
					}
					else
						L->next->coef += s->coef;//二者相加
					flag = false;
					break;
				}
				L = L->next;
